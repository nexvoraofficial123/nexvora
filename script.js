...
'use strict';
// ============================================================
// NEXVORA GOOGLE SHEETS ORDER SYSTEM
// ============================================================
...
function makeProductSummary() {
  return cart.map(item => {
    const product = PRODUCTS.find(p => p.id === item.id);
    const options = item.options || {};

    return [
      `${product ? product.name : 'Unknown product'} x${item.qty}`,
      `Color: ${options.color || ''}`,
      `Size: ${options.size || ''}`
    ].join(', ');
  }).join(' | ');
...
if (checkoutBtn) {
  checkoutBtn.onclick = async event => {
    ...
    try {
      ...
      const savedOrder = await saveOrderToGoogleSheet(orderData);
      ...
      messageLines.unshift(`Order ID: ${savedOrder.orderId}`);
      messageLines.push('');
      messageLines.push(
        'Please keep this Order ID to track your delivery.'
      );
      ...
      showToast('Could not save the order');
    }
  };
}
}
